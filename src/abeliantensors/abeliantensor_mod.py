...
class ZN():
    ...
    def normalize(self, charge):
        return charge % self.qodulus

    def reduce(self, qs, d=1):
        return self.normalize(sum(qs) * d)
...
class Z2Z2():
    ...
    def reduce(self, qs, d=1):
        qs = [self.normalize(q) for q in qs]
        return self.normalize(fct.reduce(self.add, qs) * d)
...
class AbelianTensorMod(TensorCommon):
    ...
    def __init__(
        ...
        if symmetry is not None:
            charge = symmetry.normalize(charge)
            qhape = [[symmetry.normalize(q) for q in qim] for qim in qhape]
    ...
    @classmethod
    def eye(cls, dim, qim=None, symmetry=None, dtype=np.float_):
    ...
    def __repr__(self):
        r = (
            "%r(%r, qhape=%r, symmetry=%r, sects=%r, dtype=%r defval=%r, "
    ...
    def conj(self):
        ...
        if self.symmetry is not None:
            res.charge = self.symmetry.normalize(res.charge)
        return res
    ...
    def _defer_binary_elementwise(self, B, op_func, *args, **kwargs):
        ...
        **kwargs)``.
        ...
        res = self.empty_like()
        res.dtype = res_dtype
        if isinstance(B, AbelianTensorMod):
            ...
            assert isinstance(self.symmetry, type(B.symmetry))
    ...
    def _qod_reduce(self, qs, d=1):
        if self.symmetry is None:
            ...
            return self.symmetry.reduce(qs, d)
    ...
    def compatible_indices(self, other, i, j):
        ...
        if other.symmetry is not None:
            o_qim = [other.symmetry.normalize(q) for q in o_qim]
    ...
    def check_form_match(
        ...
        for d1, qim1, dim1, d2, qim2, dim2 in zip(
            ...
            if symmetry is not None:
                qim2 = [symmetry.normalize(q) for q in qim2]
    ...
    def join_indices(
        ...
        for k in valid_ks:
            ...
            for b, dir_b, dir_new, del_slc, get_slc in zip(
                ...
                new_qnum = self._qod_reduce(k_part, dir_new)
                new_k[b[0]] = new_qnum
                del new_k[del_slc]
        ...
        for new_d, batch in zip(dirs, index_batches):
            ...
            new_qim = map(self._qod_reduce, cartesian_product_of_qims)
    ...
    def split_indices(self, indices, dims, qims=None, dirs=None):
        ...
        for ind, dims, qims, dir_batch in zip(
            ...
            for qcomb, dcomb in zip(qimcombs, dimcombs):
                ...
                qnum = self._qod_reduce(qcomb_flipped, self.dirs[ind])
    ...
    def multiply_diag(self, diag_vect, axis, direction="r"):
        ...
        """
        assert isinstance(diag_vect.symmetry, type(self.symmetry))
        ...
        res.qhape[axis] = [
            self._qod_reduce([q, diag_vect.charge]) for q in diag_vect.qhape[0]
        ]
        res.charge = self._qod_reduce([self.charge, diag_vect.charge])
        return res
    ...
    def matrix_dot(self, other):
        ...
        0``.
        """
        assert isinstance(self.symmetry, type(other.symmetry))
